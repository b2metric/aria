...
- [x] **Step 1: Real KMS providers (backend)** — `crypto.py` now has a `KEKProvider` base + real `AwsKmsProvider` (boto3 `kms.encrypt`/`decrypt`), `GcpKmsProvider` (google-cloud-kms `encrypt`/`decrypt`), `AzureKmsProvider` (azure-keyvault-keys `wrap_key`/`unwrap_key`) — each wraps/unwraps the per-customer DEK via the external KMS using the configured key URI/ARN. `get_kek_provider()` dispatches; `rewrap_dek()` migrates a DEK across providers. `get_fernet_for_customer` (sync + async) now resolves ALL providers (not just `app`). SDKs are **lazy-imported** (the seam is `_client()`), so the module loads without them and raises a clear `KmsError` naming the missing package; install the one for your cloud (`boto3` / `google-cloud-kms` / `azure-keyvault-keys`). PATCH validates the key first (wrap→unwrap probe) and **fails fast 400** if unreachable; `app` remains the default fallback. **(RESOLVED)**
- [x] **Step 2: Key lifecycle UI (frontend)** — `GET /encryption/status` (provider + live `reachable` probe), `POST /encryption/rotate` (re-wrap DEK under latest key version), and a **CMEK audit trail** (`cmek_config_change` / `cmek_key_rotation` written via `AuditService`, DEK cache evicted on change). `/settings/encryption` page now shows a **status card** (provider + reachable badge + Rotate key button) and a **Key Configuration History** list. **(RESOLVED)**
- [x] **Step 3: Tests** — 9 unit tests in `backend/tests/test_crypto_kms.py` (provider selection + unknown raises; AWS/GCP/Azure wrap→unwrap round-trips with mocked SDK clients; bad-roundtrip validation = False; `app` fallback; `rewrap_dek` app→aws). Full backend suite `97 passed`. **(RESOLVED)**

> **Verification (run-evidence):** ruff + eslint + tsc clean. **Live (aria-backend + aria-frontend, real stack):** `GET /status` → app `reachable:true`; `PATCH provider=aws` (fake key) → `400` fail-fast; invalid provider → `400`; `POST /rotate` → `200` (2 `cmek_key_rotation` audit rows confirmed); in-container rewrap proof — a Fernet secret encrypted under the DEK still decrypts after rotation. Frontend visually verified logged-in (status card + reachable badge + Rotate key + Key Configuration History showing the rotation entries).

> Note: the existing config page (provider dropdown + key URI + PATCH) already existed — this task added the *management layer* (real KMS wiring + rotation/status/audit), not a from-scratch build.

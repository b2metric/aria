...
enriched_at: '2026-06-28T02:59:22.814348+00:00'
...
Monthly **billed charges** (`SUM(BILLAMOUNT)`) broken down by region, last 6 months. Region lives
in `FCT_PREP_MASTER`, so JOIN on `CONTRNO`; bucket by `TRUNC(LOGDATE,'MM')`. Note: this is *billed
revenue* (charges), **not** prepaid top-ups — for recharge/top-up "revenue" use
`FCT_PREP_RECHARGE.TOPUP_AMOUNT` instead. The "REV" in the table name is misleading here.

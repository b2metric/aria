...
down_revision: str | None = "88b86ace576f"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            ...
            server_default="app",
            nullable=False,
            comment="app, aws, gcp, azure",
        ),
        sa.Column(
            "key_uri",
            sa.String(length=512),
            nullable=True,
            comment="URI to the KEK in the provider",
        ),
        sa.Column(
            "encrypted_dek",
            sa.Text(),
            nullable=False,
            comment="Data Encryption Key encrypted by KEK",
        ),
    ...
    op.create_index(
        op.f("ix_customer_key_configs_customer_id"),
        "customer_key_configs",
        ["customer_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_customer_key_configs_id"), "customer_key_configs", ["id"], unique=False
    )

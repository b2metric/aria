...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("role", postgresql.ENUM("admin", "member", "viewer", name="user_role", create_type=False),
    ...
    op.create_table(
        ...
        sa.Column("status", postgresql.ENUM("pending", "running", "completed", "failed", "cancelled",
                                             name="query_status", create_type=False),
                  server_default="pending"),
        sa.Column("execution_time_ms", sa.Integer(), nullable=True),
        sa.Column("rows_returned", sa.Integer(), nullable=True),
        sa.Column("error_message", sa.Text(), nullable=True),
        sa.Column("db_config_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("customer_db_configs.id", ondelete="SET NULL"), nullable=True, index=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
    )
    ...
    op.create_table(
        ...
        sa.Column("query_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("queries.id", ondelete="CASCADE"), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column("type", postgresql.ENUM("chart", "dashboard", "report", "insight",
                                           name="artifact_type", create_type=False),
                  nullable=False),
        sa.Column("status", postgresql.ENUM("draft", "published", "archived",
                                             name="artifact_status", create_type=False),
    ...
    op.create_table(
        ...
        sa.Column("customer_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("customers.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("type", postgresql.ENUM("schema_sync", "insight_gen", "report_gen", "data_export",
    ...
    op.create_table(
        ...
        sa.Column("customer_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("customers.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("type", postgresql.ENUM("sql_pattern", "business_term", "metric_def", "relationship_def",
                                           name="knowledge_type", create_type=False),
                  nullable=False),
        sa.Column("title", sa.String(500), nullable=False),
        sa.Column("content", sa.Text(), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("customer_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("customers.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("db_config_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("customer_db_configs.id", ondelete="CASCADE"), nullable=False, index=True),
        ...
        sa.Column("relationship_type", sa.String(50), server_default="inferred",
    ...
    op.create_table(
        ...
        sa.Column("period", postgresql.ENUM("daily", "monthly", name="quota_period", create_type=False),
    ...
    # ── Composite indexes ────────────────────────────────────────────
    op.create_index("ix_token_usage_daily_user_date", "token_usage_daily",
                    ["user_id", "usage_date"], unique=False)
    op.create_index("ix_memory_entries_customer_key", "memory_entries",
                    ["customer_id", "key"], unique=False)
    op.create_index("ix_vault_knowhow_customer_type", "vault_knowhow",
                    ["customer_id", "type"], unique=False)

...
down_revision: str | None = "46493ab3afbd"
...
def upgrade() -> None:
    ...
    llm_provider = postgresql.ENUM(
        "openai", "azure", "anthropic", "gemini", "litellm", name="llm_provider"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "encrypted_upstream_api_key", sa.Text(), nullable=False, comment="Encrypted at rest"
        ),
        ...
        sa.Column(
            ...
            nullable=True,
            comment="Azure deployment name or API version",
        ),
        sa.Column(
            "encrypted_virtual_key",
            sa.Text(),
            nullable=True,
            comment="LiteLLM proxy key (encrypted)",
        ),
        ...
        sa.Column(
            ...
            nullable=True,
            comment="Extra provider params as JSON",
        ),
    ...
    op.create_index(
        op.f("ix_customer_llm_configs_customer_id"),
        "customer_llm_configs",
        ["customer_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_customer_llm_configs_id"), "customer_llm_configs", ["id"], unique=False
    )

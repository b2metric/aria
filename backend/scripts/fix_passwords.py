...
async def main():
    ...
    async with async_session() as session:
        ...
        res = await session.execute(
            text("SELECT id, customer_id, encrypted_password FROM customer_db_configs")
        )
            ...
            if enc_pwd.startswith("gAAAAA"):
                try:
                    ...
                    await session.execute(
                        text(
                            "UPDATE customer_db_configs SET encrypted_password = :ep WHERE id = :id"
                        ),
        ...
        res = await session.execute(
            text(
                "SELECT id, customer_id, encrypted_upstream_api_key, encrypted_virtual_key FROM customer_llm_configs"
            )
        )
        configs = res.fetchall()

...
def verify_read_only_sql(sql: str) -> None:
    ...
    for pattern in unsafe_keywords:
        if re.search(pattern, sql_upper):
            raise ValueError(
                "Security Exception: Unsafe SQL keyword detected. Only read-only queries are allowed."
            )
    ...
    if first_word not in ("SELECT", "WITH", "EXPLAIN"):
        raise ValueError(
            f"Security Exception: SQL must begin with SELECT or WITH. Found: '{first_word}'"
        )

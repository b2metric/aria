...
class TokenQuotaBase(BaseModel):
    ...
    team_id: uuid.UUID | None = Field(
        default=None,
        description="Scope to a specific team (NULL for customer-wide or user-specific)",
    )
    user_id: uuid.UUID | None = Field(
        default=None, description="Scope to a specific user (NULL for customer-wide or team-wide)"
    )
...
class TokenQuotaUpdate(BaseModel):
    ...
    token_limit: int | None = Field(
        default=None, ge=1, description="Number of tokens allowed per period"
    )

...
class VaultSyncService:
        ...
        if db_type_str == "oracle":
            ...
            for row in rows:
                ...
                tables[t_name].append(
                    {"name": c_name, "type": c_type, "nullable": nullable, "is_pk": False}
                )
        else:
            ...
            for row in rows:
                ...
                t_name = (
                    ...
                    else getattr(row, "table_name", None)
                )
                if not t_name:
                    continue
                c_name = (
                    row.get("column_name")
                    if isinstance(row, dict)
                    else getattr(row, "column_name", None)
                )
                c_type = (
                    row.get("data_type")
                    if isinstance(row, dict)
                    else getattr(row, "data_type", None)
                )
                null_val = (
                    row.get("is_nullable")
                    if isinstance(row, dict)
                    else getattr(row, "is_nullable", "YES")
                )
            ...
            try:
                ...
                for line in body.split("\n"):
                    if (
                        ...
                        and not line.strip().startswith("| Column")
    ...
    def _update_markdown_if_changed(
        self, table_name: str, live_columns: list, existing: dict
    ) -> bool:
        ...
        for col in live_columns:
            ...
            new_table_lines.append(
                f"| {col['name']} | {col['type']} | {null_mark} | {pk_mark} | {desc} |"
            )

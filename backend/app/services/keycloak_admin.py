...
class KeycloakAdminService:
    ...
    async def _get_admin_token(self, client: httpx.AsyncClient) -> str:
        ...
        if not response.is_success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to authenticate with Identity Provider"
            )
    ...
    async def create_user(
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 409:
                raise HTTPException(
                    status_code=409, detail="User with this email already exists in IdP"
                )
            ...
            if not location:
                ...
                get_resp = await client.get(
                    f"{users_url}?username={email}", headers=self._auth_header(token)
                )
                users = get_resp.json()
            ...
            await client.put(
                f"{users_url}/{kc_user_id}", json=update_payload, headers=self._auth_header(token)
            )

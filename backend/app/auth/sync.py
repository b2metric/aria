...
import uuid
from backend.app.models.organization import User, Team, Customer
        ...
        async with maker() as db:
            ...
            if user_ctx.team_id:
                try:
                    ...
                    team_stmt = insert(Team).values(
                        id=team_uuid,
                        customer_id=uuid.UUID(user_ctx.workspace_id) if user_ctx.workspace_id != "default" else None, # We need a valid customer ID if possible
                    ...
                    logger.debug(f"Could not upsert team from token (might lack customer mapping): {e}")
                    await db.rollback()
            ...
            if user_ctx.workspace_id:
                ...
                try:
                    ...
                    # It's a slug
                    res = await db.execute(select(Customer.id).where(Customer.slug == user_ctx.workspace_id))

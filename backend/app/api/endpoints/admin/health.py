        ...
        async with get_engine().connect() as conn:
            ...
            if not conf_rows:
                results["customer_dbs"] = {
                    "status": "healthy",
                    "error": "No active customer DBs configured yet",
                }
            else:
                db_success = 0
                db_errors = []
                ...
                for c in conf_rows:
                    db_type_val = (
                        c.db_type
                        if hasattr(c, "db_type") and isinstance(c.db_type, str)
                        else (c.db_type.value if hasattr(c.db_type, "value") else str(c.db_type))
                    )
                    c_config = DBConfig(
                        db_type=db_type_val,
                        host=c.host,
                        ...
                        password=await async_decrypt_password(
                            c.encrypted_password, c.customer_id, conn
                        ),
                    )
                    get_executor(c_config)
                    ...
                    try:
                        ...
                        await loop.run_in_executor(
                            None,
                            lambda conf=c_config, dt=db_type_val: get_executor(conf).execute(
                                "SELECT 1" if dt != "oracle" else "SELECT 1 FROM DUAL"
                            ),
                        )
                        db_success += 1
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        ...
        try:
            ...
            if resp.status_code in [
                200,
                404,
            ]:  # 404 might happen if endpoint varies, but connection succeeds
                results["qdrant"] = {
                    "status": "healthy",
                    "latency_ms": round((asyncio.get_event_loop().time() - start) * 1000),
                }
        ...
        try:
            ...
            latency_ms = round((asyncio.get_event_loop().time() - start) * 1000)
            if resp.status_code == 200:
                results["litellm"] = {"status": "healthy", "latency_ms": latency_ms}
            else:
                results["litellm"] = {
                    "status": "unhealthy",
                    "latency_ms": latency_ms,

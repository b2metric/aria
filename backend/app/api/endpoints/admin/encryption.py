...
async def update_encryption_config(
    ...
    if body.provider not in ["app", "aws", "gcp", "azure"]:
        raise HTTPException(
            status_code=400, detail="Invalid provider. Must be one of: app, aws, gcp, azure"
        )
        ...
        async with get_sessionmaker()() as session:
            ...
            if config:
                ...
                if config.provider != body.provider:
                    log.info(
                        f"Tenant {workspace_id} switching KEK provider from {config.provider} to {body.provider}"
                    )

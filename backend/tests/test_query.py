...
def jwks(rsa_key):
    ...
    return {"keys": [{"kty": "RSA", "kid": "test-kid", "use": "sig", "alg": "RS256",
            "n": _b64url_int(pub_numbers.n), "e": _b64url_int(pub_numbers.e)}]}
...
def auth_token(rsa_key):
    now = int(time.time())
    payload = {
        "sub": "test-user-uuid", "iss": "http://localhost:8080/realms/aria",
        "aud": "aria-backend", "exp": now + 3600, "iat": now,
        "workspace_id": "ws-test-001", "user_id": "user-001",
        "team_id": "team-001", "role": "analyst",
        "email": "test@b2metric.com", "name": "Test User",
...
async def _mock_process_query(redis, engine, request, workspace_id, user_id):
    ...
    yield {"event": "status", "data": json.dumps({"status": "generating_sql", "message": "Generating SQL..."})}
    yield {"event": "sql", "data": json.dumps({
        "sql": "SELECT category, SUM(value) AS total FROM sales GROUP BY category",
        "explanation": "Grouped sales by category.",
    })}
    yield {"event": "status", "data": json.dumps({"status": "sql_ready", "message": "SQL generated"})}
    yield {"event": "status", "data": json.dumps({"status": "executing", "message": "Executing..."})}
    yield {"event": "status", "data": json.dumps({
        "status": "rendering_chart", "message": "Building chart from 3 rows..."})}
    yield {"event": "chart", "data": json.dumps({
        "chart_type": "bar",
    ...
    yield {"event": "status", "data": json.dumps({
        "status": "complete", "message": "Done", "conversation_id": cid})}
